 ...
 * Time: 14:04
 */
public abstract class MultiIntentService extends BaseService {
    // 默认空闲5分钟后自动stopSelf()
    ...
    public void onDestroy() {
        super.onDestroy();
        L.d("mRetainCount=" + mRetainCount.get());
        L.d("mFutures.size()=" + mFutures.size());
        cancelAutoClose();
        destroyHandler();
        destroyExecutor();
    }
    ...
    private void dispatchIntent(final Intent intent) {
        ...
        final Runnable runnable = new Runnable() {
            @Override
            public void run() {
                L.d("dispatchIntent thread=" + Thread.currentThread());
                L.d("dispatchIntent start tag=" + tag);
                onHandleIntent(intent, tag);
                L.d("dispatchIntent end tag=" + tag);
                release(tag);
            }
        };
    ...
    private void autoClose() {
        L.d("autoClose() mRetainCount=" + mRetainCount.get());
        L.d("autoClose() mFutures.size()=" + mFutures.size());
        if (isIdle()) {
            stopSelf();
        }
    }
